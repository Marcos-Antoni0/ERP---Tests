...
| | T15.2 | Melhorar **layout da aba Configurações** (Adicionar um navbar das opções em configurações e redirecionar para cada tela conforme a opção selecionada). | [ ] |
| **T16.0** | **Fechamento de Caixa** | Ajustes finais no relatório de fechamento. | [ ] |
| | T16.1 | Validar e testar a exibição correta de **vendas débito** separadas do **total vendido e apurado**. | [ ] |
