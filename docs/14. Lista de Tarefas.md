...
| **T12.0** | **Débitos** | Melhorias na funcionalidade de Débitos. | [ ] |
| | T12.1 | Implementar **Campo de pesquisa por data em débitos**. | [ ] |
| | T12.2 | Corrigir *bug* em **Editar débito** (fica sem valor no campo ao clicar em editar). | [ ] |
| **T13.0** | **Estoque** | Ajuste na lógica de estoque. | [ ] |
| | T13.1 | Permitir realizar **vendas, pedidos ou adicionar em comanda** para venda mesmo com **estoque negativo** (ajuste na validação de estoque). | [ ] |
| **T14.0** | **Vendas/Checkout** | Ajustes no fluxo de vendas. | [ ] |
| | T14.1 | Ajustar a **Posição do componente seleção de cliente** em *checkout* para melhor usabilidade. | [ ] |
| **T15.0** | **Configurações** | Adicionar novas opções de personalização. | [ ] |
| | T15.1 | Botar em **Configurações** a opção de **abrir tela de impressão ou não** (toggle). | [ ] |
| | T15.2 | Melhorar **layout da aba Configurações** (ir adicionando configurações para melhor personalização). | [ ] |
| **T16.0** | **Fechamento de Caixa** | Ajustes no relatório de fechamento. | [ ] |
| | T16.1 | Adicionar em **fechamento de caixa** o valor de **vendas débito** e **separar do total vendido e apurado**. | [ ] |
| **T17.0** | **Operador** | Ajuste na exibição de informações. | [ ] |
| | T17.1 | Em tela de **operador**, adicionar o **nome** ao invés do usuário (username). | [ ] |
