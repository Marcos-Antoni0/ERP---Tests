...
| | T4.1 | Corrigir o *trigger* de impressão automática para disparar o `receipt_caixa.html` ao finalizar a venda no PDV. | [ ] |
| | T4.2 | Criar a função de impressão que lê a impressora padrão configurada. | [ ] |
| | T4.3 | Integrar a função de impressão no fluxo de finalização de venda, comandas e pedidos. | [ ] |
| **T4.4** | **Impressão Manual** | Corrigir o layout de impressão manual em todos os recibos (exceto o automático) para evitar corte de informações e unificar o layout com `receipt_caixa.html`. | [ ] |
| **T4.5** | **Layout XML** | Aumentar o tamanho dos campos de custo e preço no modal de pré-visualização XML para visualização completa dos valores. | [ ] |
| **T4.6** | **Lógica XML** | Implementar a lógica de preenchimento automático: custo com valor unitário da nota (XML) e preço com valor de venda do produto. | [ ] |
