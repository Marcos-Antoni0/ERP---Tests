...
def _normalize_header(value):
    normalized = (
        unicodedata.normalize("NFKD", str(value or "")).encode("ASCII", "ignore").decode("ASCII")
    )
...
def estoque(request):
    ...
    estoque_qs = base_qs.filter(produto__name__icontains=query) if query else base_qs
    ...
    total_value = base_qs.aggregate(total=Sum(F("quantidade") * F("produto__price")))["total"] or Decimal("0")
    total_cost = base_qs.aggregate(total=Sum(F("quantidade") * F("produto__custo")))["total"] or Decimal("0")
...
def manage_products_estoque(request):
    ...
    if prod_id.isnumeric():
        product = get_object_or_404(Estoque, pk=int(prod_id), company=user_company)
...
def save_product_estoque(request):
    ...
    try:
        produto = get_object_or_404(Products, pk=int(produto_id), company=user_company)
        categoria = get_object_or_404(Category, pk=int(categoria_id), company=user_company)
    ...
    if record_id.isnumeric() and int(record_id) > 0:
        estoque = get_object_or_404(Estoque, pk=int(record_id), company=user_company)
    else:
...
def upload_estoque(request):
    ...
    validity_idx = resolve_index(["validade", "validade (dias)", "validade em dias"])
    ...
    for row_number, row in enumerate(rows[1:], start=2):
        ...
        category_cell = row[category_idx] if category_idx is not None and len(row) > category_idx else None
        quantity_cell = row[quantity_idx] if len(row) > quantity_idx else None
        validity_cell = row[validity_idx] if validity_idx is not None and len(row) > validity_idx else None
        ...
        if quantity_value is None:
            error_rows.append(f"Linha {row_number}: informe uma quantidade válida.")
            continue
        ...
        try:
            estoque_obj = Estoque.objects.filter(company=user_company, produto=product).first()
            if estoque_obj:
                ...
                Estoque.objects.create(
                    ...
                    preco=float(price_value) if price_value is not None else float(product.price or 0),
                    custo=float(cost_value) if cost_value is not None else float(product.custo or 0),
                    status=status_value,
                    descricao=product,
                )
                created_count += 1
        except Exception as exc:
            error_rows.append(f"Linha {row_number}: erro ao salvar estoque ({exc}).")
            continue
