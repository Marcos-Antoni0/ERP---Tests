...
class Products(TenantMixin):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.category_id and self.company_id and self.category_id.company_id != self.company_id:
            raise ValueError("A categoria deve pertencer à mesma empresa do produto")
...
class ProductComboItem(TenantMixin):
    ...
    def clean(self):
        ...
        if self.combo_id and not self.combo.is_combo:
            raise ValidationError("O produto selecionado não está configurado como combo.")
        if self.combo_id and self.component_id and self.combo_id == self.component_id:
            raise ValidationError("O combo não pode incluir ele mesmo como componente.")
        if (
            self.component_id
            and self.company_id
            and self.component.company_id != self.company_id
        ):
            raise ValidationError("O componente precisa pertencer à mesma empresa do combo.")
...
class Sales(TenantMixin):
    customer_name = models.CharField("Nome do Cliente", max_length=100, blank=True, null=True)
    code = models.CharField(max_length=100)
    ...
    discount_reason = models.CharField("Motivo do desconto", max_length=255, blank=True)
...
class PedidoItem(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product and self.pedido and self.product.company_id != self.pedido.company_id:
            raise ValueError("O produto deve pertencer à mesma empresa do pedido")
        super().save(*args, **kwargs)
...
class salesItems(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if self.product_id and self.sale_id and self.product_id.company_id != self.sale_id.company_id:
            raise ValueError("O produto deve pertencer à mesma empresa da venda")
        super().save(*args, **kwargs)
...
class Estoque(TenantMixin):
    ...
    descricao = models.ForeignKey(Products, on_delete=models.CASCADE, related_name='estoque_descricao', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        ...
        if self.produto and self.company_id and self.produto.company_id != self.company_id:
            raise ValueError("O produto deve pertencer à mesma empresa do estoque")
        if self.categoria and self.company_id and self.categoria.company_id != self.company_id:
            raise ValueError("A categoria deve pertencer à mesma empresa do estoque")
        super().save(*args, **kwargs)
    ...
    def clean(self):
        ...
        if self.waiter_id:
            ...
            if (
                ...
                raise ValidationError("O garçom deve pertencer à mesma empresa da mesa")
...
class TableOrder(TenantMixin):
    ...
    payment_method = models.CharField(max_length=10, choices=Sales.FORMA_PAGAMENTO_CHOICES, blank=True)
    notes = models.TextField(blank=True)
    ...
    def clean(self):
        ...
        if self.table_id:
            table_company_id = Table.objects.filter(pk=self.table_id).values_list("company_id", flat=True).first()
        ...
        if (
            ...
            raise ValidationError("A mesa deve pertencer à mesma empresa da comanda")
        ...
        if self.waiter_id:
            ...
            if (
                ...
                raise ValidationError("O garçom deve pertencer à mesma empresa da comanda")
    ...
    def recalculate_totals(self, commit=True):
        subtotal = self.items.aggregate(total=Sum('total'))['total'] or Decimal('0.00')
...
class TableOrderItem(models.Model):
    order = models.ForeignKey(TableOrder, related_name='items', on_delete=models.CASCADE)
    ...
    def clean(self):
        if self.product_id and self.order_id:
            ...
            if (
                ...
                raise ValidationError("O produto deve pertencer à mesma empresa da comanda")

    def save(self, *args, **kwargs):
        self.unit_price = Decimal(self.unit_price)
        self.quantity = Decimal(self.quantity)
        self.total = (self.unit_price * self.quantity).quantize(Decimal('0.01'))
        super().save(*args, **kwargs)
...
class CashMovement(TenantMixin):
    ...
    MOVEMENT_PAYMENT_CHOICES = Sales.FORMA_PAGAMENTO_CHOICES + [("AJUSTE", "Ajuste manual")]
    ...
    type = models.CharField(max_length=5, choices=Type.choices, db_column="movement_type")
