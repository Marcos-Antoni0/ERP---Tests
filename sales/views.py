...
def save_pos(request):
    ...
    def resolve_combo_components(product, raw_config, combo_qty):
        ...
        for entry in config_entries:
            ...
            if component_id not in allowed_components:
                raise ValueError("Um dos itens informados não pertence a este combo.")

            try:
                per_combo_qty = Decimal(str(entry.get("quantity", 0)))
            except (InvalidOperation, ValueError):
                raise ValueError("Quantidade inválida para um dos componentes do combo.")

            if per_combo_qty < 0:
                raise ValueError("Quantidade do componente do combo não pode ser negativa.")
            ...
            payload = aggregated.setdefault(
                component_id,
                {"item": allowed_components[component_id], "per_combo": Decimal("0")},
            )
    ...
    try:
        ...
        delivery_value = Decimal(str(data.get("taxa_entrega", data.get("delivery_fee", 0)) or 0))
    ...
    base_for_discount = sub_total_value + tax_amount_value + max(delivery_value, Decimal("0"))
    ...
    grand_total_value = sub_total_value + tax_amount_value + delivery_value - discount_value
        ...
        try:
            with transaction.atomic():
                ...
                for idx, prod_id in enumerate(data.getlist("product_id[]")):
                    product = Products.objects.get(id=prod_id, company=user_company)
                    qty_raw = data.getlist("qty[]")[idx]
                    try:
                        qty_decimal = Decimal(str(qty_raw))
                    except (InvalidOperation, ValueError):
                        raise ValueError("Quantidade inválida informada para um dos itens.")
                    ...
                    if product.is_combo:
                        raw_config = combo_configs[idx] if idx < len(combo_configs) else ""
                        combo_components = resolve_combo_components(
                            product, raw_config, qty_decimal
                        )
                        PedidoComboItem.objects.filter(pedido_item=pedido_item).delete()
                ...
                resp = {"status": "success", "sale_id": pedido.id, "type": "pedido"}
    ...
    try:
        ...
        try:
            payment_entries = parse_payment_entries(payment_methods, payment_amounts)
        ...
        with transaction.atomic():
                ...
                if product.is_combo:
                    ...
                    for component in combo_components:
                        ...
                        try:
                            ...
                            estoque_item.quantidade -= float(component["total_quantity"])
            ...
            resp = {"status": "success", "sale_id": venda.id, "type": "venda"}
    except Exception as exc:
...
def salesList(request):
    ...
    try:
        filter_start = datetime.strptime(start_date, "%Y-%m-%d").date() if start_date else default_start
    except ValueError:
        filter_start = default_start

    try:
        filter_end = datetime.strptime(end_date, "%Y-%m-%d").date() if end_date else today
    except ValueError:
        filter_end = today
    ...
    sales_qs = (
        base_qs.select_related("table", "table_order__table", "table_order__waiter")
    ...
    payment_methods = (
        base_qs.values_list("forma_pagamento", flat=True).distinct().order_by("forma_pagamento")
    )
...
def receipt(request):
    ...
    sale = (
        Sales.objects.select_related("table", "table_order__table", "table_order__waiter")
        .filter(id=sale_id)
        .first()
    )
...
def cashier_dashboard(request):
    ...
    if open_session:
        movement_form = CashMovementForm(
            initial={"type": CashMovement.Type.ENTRY, "payment_method": "DINHEIRO"}
        )
        close_form = CashCloseForm(
        ...
        manual_movements = [movement for movement in session_movements if movement.sale_id is None]
        sale_ids = (
            movement.sale_id
    ...
    if history_date:
        session_history_qs = session_history_qs.filter(opened_at__date=history_date)
...
def register_cash_movement(request):
    if request.method != "POST":
        messages.error(request, "Método inválido para registrar movimentação de caixa.")
        return redirect("cashier")
...
def close_cash_session(request):
    ...
    with transaction.atomic():
        ...
        session.save(update_fields=["status", "closed_at", "closed_by", "closing_amount", "closing_note"])
...
def cashier_session_report(request, session_id):
    ...
    session = get_object_or_404(CashRegisterSession, pk=session_id, company=user_company)
    pdf_bytes = generate_cash_report_pdf(session)
...
def export_sales_report(request):
    ...
    ws.append(["Data", "Código", "Produto", "Categoria", "Quantidade", "Receita"])
    for rec in qs:
        ws.append(
            [
