...
class TableForm(forms.ModelForm):
    ...
    def clean_number(self):
        ...
        if number and number <= 0:
            raise forms.ValidationError("O número da mesa deve ser maior que zero.")
        return number

    def clean(self):
        cleaned = super().clean()
        if self.company:
            qs = Table.objects.filter(company=self.company, number=cleaned.get("number"))
            if self.instance.pk:
                qs = qs.exclude(pk=self.instance.pk)
            if qs.exists():
                raise forms.ValidationError("Já existe uma mesa com esse número nesta empresa.")
        return cleaned
...
class TableOrderForm(forms.ModelForm):
    service_charge = forms.DecimalField(
        ...
        widget=forms.NumberInput(
            attrs={"class": "form-control", "min": "0", "step": "0.1", "placeholder": "Ex.: 10 para 10%"}
        ),
    ...
    class Meta:
        model = TableOrder
        fields = ["waiter", "people_count", "service_charge", "discount_amount", "discount_reason", "notes"]
        labels = {
            "waiter": "Garçom",
    ...
    def clean(self):
        ...
        if discount > 0 and not reason:
            self.add_error("discount_reason", "Informe o motivo do desconto aplicado.")
        elif discount <= 0:
            reason = ""
...
class TableOrderCloseForm(forms.ModelForm):
    service_charge = forms.DecimalField(
        ...
        label="Taxa de serviço (%)",
        widget=forms.NumberInput(
            attrs={"class": "form-control", "min": "0", "step": "0.1", "placeholder": "Informe o percentual"}
        ),
    ...
    class Meta:
        model = TableOrder
        fields = ["service_charge", "discount_amount", "discount_reason", "notes"]
        labels = {
    ...
    def __init__(self, *args, **kwargs):
        ...
        if company is not None:
            self.fields["product"].queryset = (
                Products.objects.filter(company=company, status=1, is_combo=False)
                .order_by("name")
            )
