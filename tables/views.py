...
def mesas(request):
    ...
    if search_number:
        try:
            ...
            messages.error(request, "Informe apenas números ao buscar uma mesa.")
        else:
            if table_models_ready():
                table = Table.objects.filter(company=user_company, number=number).first()
                if table:
    ...
    if tables_ready:
        active_waiters_qs = Garcom.objects.filter(company=user_company, is_active=True).order_by("name")
...
def excluir_mesa(request, table_id):
    ...
    if table.orders.filter(status=TableOrder.Status.OPEN).exists():
        messages.error(request, "Não é possível excluir uma mesa com comanda aberta.")
    else:
        table.delete()
...
def mesa_detalhe(request, table_id):
        ...
        .prefetch_related(
            Prefetch(
                ...
                .prefetch_related(
                    ...
                    Prefetch(
                        "sales",
                        queryset=Sales.objects.filter(company=user_company).order_by("-date_added"),
                    ),
                )
                .order_by("-opened_at"),
            )
    ...
    open_order = next((order for order in table.orders.all() if order.status == TableOrder.Status.OPEN), None)
    closed_orders = []
    ...
    if open_order:
        ...
        order_form = TableOrderForm(initial={"people_count": table.capacity}, company=user_company)
...
def abrir_comanda(request, table_id):
    ...
    if table.orders.filter(status=TableOrder.Status.OPEN).exists():
        messages.warning(request, "Já existe uma comanda aberta para esta mesa.")
        return redirect("mesa-detalhe", table_id=table.id)

    if not Garcom.objects.filter(company=user_company, is_active=True).exists():
        messages.error(request, "Cadastre um garçom ativo antes de abrir uma comanda.")
        return redirect("mesa-detalhe", table_id=table.id)
...
def reabrir_venda_mesa(request, sale_id):
    ...
    if not sale_type.startswith("mesa"):
        messages.error(request, "Apenas vendas originadas de mesas podem ser reabertas.")
        return redirect("sales-page")

    if request.method != "POST":
        messages.error(request, "Método inválido para reabrir comanda da venda.")
        return redirect("sales-page")

    order = sale.table_order
    if not order:
        messages.error(request, "Esta venda não possui comanda vinculada para reabertura.")
        return redirect("sales-page")

    status, feedback = reopen_table_order(order, user_company)
    if status == "success":
        messages.success(request, f"{feedback} A venda #{sale.code} foi revertida.")
    elif status == "info":
        messages.info(request, feedback)
    else:
        messages.error(request, feedback)
...
def excluir_comanda(request, order_id):
    ...
    order = TableOrder.objects.select_related("table").filter(pk=order_id).first()
...
def adicionar_item_comanda(request, order_id):
    ...
    if order.status != TableOrder.Status.OPEN:
        messages.error(request, "Não é possível alterar itens de uma comanda fechada.")
        return redirect("mesa-detalhe", table_id=order.table_id)
...
def atualizar_item_comanda(request, item_id):
    ...
    item = get_object_or_404(
        TableOrderItem.objects.select_related("order", "order__table", "product"),
        pk=item_id,
    ...
    form = TableOrderItemForm(request.POST, instance=item, company=user_company)
    if form.is_valid():
        item = form.save(commit=False)
        product = item.product
        if product.status != 1:
...
def toggle_product_availability(request, product_id):
    ...
    if request.method != "POST":
        messages.error(request, "Método inválido para atualizar disponibilidade.")
        return redirect("mesas")
    ...
    redirect_target = request.POST.get("next") or request.META.get("HTTP_REFERER") or reverse("mesas")
    return redirect(redirect_target)
